...
@(define talk2
   @make-talk{Performance Analysis of Zippers
              ...
              @abstract{The zipper is a powerful technique of representing purely functional data structures in a way that allows fast access to a specific element. It is often used in cases where the imperative approach would use a mutable pointer. However, it is unclear whether zippers offer the same performance benefit of mutable pointers. We attempt to answer this question by comparing the performance gain of zippers and mutable pointers in two common scenarios and three different languages: C++, C#, and Haskell.}
              @resources[#f #f #f]})
...
@(define talk6
   @make-talk{How to Specify it! A Guide to Writing Properties of Pure Functions.
              ...
              @abstract{Many developers trying property-based testing for the first time find it difficult to identify properties to write—and find the simple examples in tutorials difficult to generalize. This is known as the oracle problem, and it is common to all approaches to testing that use test case generation. In this paper, therefore, we take a simple—but non-trivial—example of a purely functional data structure, and present five different approaches to writing properties, along with the pitfalls of each to keep in mind. We compare and constrast their effectiveness with the help of eight buggy implementations. We hope that the concrete advice presented here will enable readers to side-step the “where do I start?” question, and quickly derive the benefits that property-based testing has to offer.}
              @resources[#f #f #f]})
...
@(define talk8
   @make-talk{Set Constraints, Pattern Match Analysis, and SMT
              ...
              @abstract{We present a translation that converts a set constraint formula into an SMT problem. Our technique allows for arbitrary boolean combinations of positive or negative set constraints,and leverages the performance of modern solvers such as Z3. To show the usefulness of unrestricted set constraints,we use them to devise a pattern match analysis for functional languages. This analysis ensures that missing cases of pattern matches are always unreachable. We implement our analysis in the Elm compiler and show that the our translation is fast enough to be used in practical verification.}
              @resources[#f #f #f]})
